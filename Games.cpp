                ...
                if (event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    if (v_board.pieces[pos.sq1] != nullptr)
                    {
                        ...
                        while (event.type != sf::Event::MouseButtonReleased)
                        {
                            ...
                            v_board.displayBoard(window, board, sf::Color(209, 139, 71), sf::Color(255, 206, 158), move_list_highlight);
...
game_outcomes Games::AI(sf::RenderWindow &window)
{
    ...
    while (window.isOpen())
    {
        ...
        if ((board.rights & 1) == computer_player)
        {
            ...
            auto comp_move = computer_move.generateComputerMove(board, promote_to, DEPTH);
